...
"unrankpap 0 q ys k = Nil"
|
"unrankpap (Suc n) q ys k = (((unrank (length ys) q (k mod q^(length ys)))!(Suc n))*((unrank ((Suc n)-(length ys)) q ((k div q^(length ys))+1))!(Suc n)) )# (unrankpap n q ys k)" 
